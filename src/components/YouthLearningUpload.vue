...
export default {
  ...
  methods: {
    ...
    async judgeRepeat(){
      var class_id;
      await this.$axios.get(this.global.server_config.url + "/upload/getClassId", {
          params: {
        ...
        .then((res1) => {
          ...
          // 判断这个班级是否已经上传了文件
          this.$axios
            .get(this.global.server_config.url + "/upload/judgeUpload", {
              params: {
                classId: class_id,
    ...
    get_class_id(){
      var than = this;
      this.$axios.get(this.global.server_config.url + "/upload/getClassId", {
          params: {
    ...
    //合成需要的url
    syntheticUploadUrl(){
      var url = this.global.server_config.url + "/upload/SingleFile" 
...
// **************** 选择框信息获取 ****************
    ...
    choise_grade(event) {
      var than = this;
      this.$axios
        .get(this.global.server_config.url + "/upload/getMajor", {
          params: {
            grade: event,
          },
          headers: {
    ...
    choise_major(event) {
      var than = this;
      this.$axios
        .get(this.global.server_config.url + "/upload/getAllClass", {
          params: {
            grade: than.$data.grade_value,
            major: event,
          },
